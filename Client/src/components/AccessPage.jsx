...
const AccessPage = () => {
    ...
    const handleSubmit = async () => {
        ...
        if (response.ok) {
            sessionStorage.setItem("noirToken", data.token);
        ...
        <div className="relative flex h-screen w-full flex-col items-center justify-between overflow-hidden px-6 py-12 md:py-24">
            ...
            <div className="flex flex-col items-center w-full max-w-[380px] space-y-8 mb-20">
                ...
                <div className='w-full px-4 pt-4'> 
                    <div onClick={handleSubmit} className='block w-full bg-primary hover:primary/90 text-white font-medium py-4 rounded transition-color uppercase tracking-[0.2em] text-xs shadow-lg shadow-primary/10 truncate uppercase w-full cursor-pointer flex items-center justify-center rounded h-14 bg-primary text-white text-sm font-bold tracking-[0.2em] transition-all hover:bg-primary/90 active:scale-95 shadow-lg shadow-primary/20'
                        >
                        Validate Invite
                    </div>
                </div>
